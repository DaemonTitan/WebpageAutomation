...
* 8. Check guest field level validation
*/
...
test.beforeEach(async({page, homePage, menuPage, productPage, mycartPage}) => {
    await page.goto('/');
...
test('Field level validation', async({checkoutPage}) => {
    ...
    await checkoutPage.validatePhoneNumberRequires04Prefix(
        fieldValidationData.PhoneField.invalidPrefix
    );

    await checkoutPage.validatePhoneNumberAccepts04Prefix(
        fieldValidationData.PhoneField.validValue
    );

    await checkoutPage.validateInvalidEmailAddress(
        fieldValidationData.EmailField.invalidValue
    );

    await checkoutPage.validateValidEmailAddress(
        fieldValidationData.EmailField.validValue
    );
})
