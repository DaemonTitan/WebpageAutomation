...
=========================== logs ===========================
waiting for navigation to "/order-summary" until "load"
...
> 13 |         await this.page.waitForURL('/order-summary', {timeout: 600000})
     |                         ^ Error: page.waitForURL: Test timeout of 30000ms exceeded.
  14 |         await expect(this.confirmationMessage).toBeVisible({ timeout: 600000 });
  15 |     }
  16 | 
  17 |     async confirmationMessageIsVisible(): Promise<void> {
  18 |         await this.waitForPageLoad();
  19 |     }
  20 | }
  21 | 
```