...
Call log:
  - waiting for locator('iframe[title=\'Iframe for card number\']').contentFrame().getByLabel('Card number')
...
```ts
  ...
  94  |         await this.addPaymentMethod.isEnabled();
  95  |         await this.addPaymentMethod.click();
  96  |     }
  97  | 
  98  |     async clickManualCardEntryButton(): Promise<void> {
  99  |         await this.manualCardEntryButton.isVisible();
  ...
  104 | 
> 105 |         await this.page.frameLocator("iframe[title='Iframe for card number']").getByLabel('Card number').fill(cardNumber);
      |                                                                                                          ^ Error: locator.fill: Target page, context or browser has been closed
  106 | 
  107 |         //await this.fillText(this.cardNumberField, cardNumber);
  108 |         // await this.fillText(this.expiryDateField, expiryDate);
  109 |         // await this.fillText(this.securityCodeField, securityCode);
  110 |     }
  111 | 
  112 |     async clickPayButton(): Promise<void> {
  113 |         await this.payButon.click();
  114 |     }
  115 | }
```