...
Expected: visible
Error: strict mode violation: getByTestId('store-name') resolved to 10 elements:
...
```ts
  ...
  34 |         this.storeNameList = page.getByTestId('store-name');
...
> 70 |         await expect(this.storeNameList).toBeVisible({ timeout: 50000 });
     |                                          ^ Error: expect(locator).toBeVisible() failed
