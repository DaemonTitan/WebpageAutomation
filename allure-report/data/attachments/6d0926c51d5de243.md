...
```ts
  ...
  57  |         await this.storeSearchField.waitFor({ state: 'visible' });
  ...
  64  |         let searchResultList = await this.suggesAddressList.all();
  65  | 
  66  |         let isFound = true;
  67  |         for(const locations of searchResultList) {
  68  |             const location = await locations.innerText();
  69  |             if (location.toLowerCase().includes(locationQuery.toLowerCase())) {
  70  |                 await locations.click();
  71  |                 isFound = true
  72  |                 break;
  73  |             }
  74  |         }
  75  |         if (!isFound) {
  76  |             throw new Error(`No store found in the search query list`)
  77  |         }
  78  |     }
  79  | 
  80  |     async selectTargetStoreFromList(locationQuery: string): Promise<void> {
> 81  |         await this.storeNameList.waitFor({ state: 'visible' });
      |                                  ^ Error: locator.waitFor: Error: strict mode violation: locator('[data-testid=\'store-name\']') resolved to 10 elements:
