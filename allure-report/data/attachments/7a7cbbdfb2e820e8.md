...
Locator: locator('iframe[title="Iframe for card number"]').contentFrame().getByRole('textbox', { name: 'Card number' })
Expected: visible
...
```ts
  ...
  103 |         await this.waitForCardEntryFields();
  104 |     }
  105 | 
  106 |     async clickManualCardEntryButton(): Promise<void> {
  107 |         await expect(this.manualCardEntryButton).toBeVisible();
  108 |         await this.manualCardEntryButton.click();
  109 |     }
  110 | 
  111 |     async enterCardDetails(cardNumber: string, expiryDate: string, securityCode: string): Promise<void> {
  112 |         //await this.waitForCardEntryFields();
  113 | 
  114 |         await this.cardNumberFrame
  115 |             .getByRole('textbox', { name: 'Card number' })
  116 |             .fill(cardNumber);
  117 | 
  118 |         await this.expiryDateFrame
  119 |             .getByRole('textbox', { name: 'Expiry date' })
  120 |             .fill(expiryDate);
  121 | 
  122 |         await this.securityCodeFrame
  123 |             .getByRole('textbox', { name: 'Security code' })
  124 |             .fill(securityCode);
  125 |     }
  126 | 
  127 |     async waitForCardEntryFields(): Promise<void> {
> 128 |         await expect(this.cardNumberField).toBeVisible({ timeout: 30000 });
      |                                            ^ Error: expect(locator).toBeVisible() failed
  129 |         await expect(this.expiryDateField).toBeVisible({ timeout: 30000 });
  130 |         await expect(this.securityCodeField).toBeVisible({ timeout: 30000 });
  131 |     }
  132 | 
  133 |     async clickPayButton(): Promise<OrderConfirmationPage> {
  134 |         await expect(this.payButon).toBeEnabled();
  135 |         await this.payButon.click();
  136 | 
  137 |         const orderConfirmationPage = new OrderConfirmationPage(this.page);
  138 |         await orderConfirmationPage.waitForPageLoad();
  139 |         return orderConfirmationPage;
  140 |     }
  141 | }
  142 | 
```