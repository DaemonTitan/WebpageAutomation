...
> 111 |         await this.cardNumberField.fill(cardNumber);
      |                                    ^ Error: locator.fill: Target page, context or browser has been closed
  112 |         await this.expiryDateField.fill(expiryDate);
  113 |         await this.securityCodeField.fill(securityCode);
  114 |     }
  115 | 
  116 |     async clickPayButton(): Promise<OrderConfirmationPage> {
  117 |         await expect(this.payButon).toBeEnabled();
  118 |         await this.payButon.click();
  119 |         return new OrderConfirmationPage(this.page);
  120 |     }
  121 | }
  122 | 
```