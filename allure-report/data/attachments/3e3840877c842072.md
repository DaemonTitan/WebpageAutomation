...
> 54  |         await this.storeSearchField.waitFor({ state: 'visible' });
  ...
  58  |         
  59  |         await this.storeSearchField.clear();
  60  |         await this.storeSearchField.fill(locationQuery);
  61  |     }
  62  | 
  63  |     async selectTargetStore(locationQuery: string): Promise<void> {
  64  |         await this.suggesAddressList.waitFor({ state: 'visible' });
  ...
  82  |         await this.storeNameList.waitFor({ state: 'visible' });
  83  |         let storeCardListArray = await this.storeNameList.all();
  84  |         let orderHerebuttonArray = await this.orderHereButton.all();
  85  |         let isFound = true;
  86  | 
  87  |         for(let i=0; i<storeCardListArray.length;i++) {
  88  |             const storeLocation = await storeCardListArray[i].innerText();
  89  |             if (storeLocation.toLowerCase().includes(locationQuery.toLowerCase())){
  90  |                 await orderHerebuttonArray[i].click();
  91  |                 isFound = true;
  92  |                 break;
  93  |             }
  94  |         }
  95  | 
  96  |         if (!isFound) {
  97  |             throw new Error(`No store found in ${locationQuery}`);
  98  |         }
  99  |     }
  100 |     async orderSummaryTitleIsVisible(): Promise<boolean> {
  101 |         return await this.orderSummaryTitle.isVisible();
  102 |     }
  103 | 
  104 |     async validateOrderSummary(): Promise<void> {
  105 |         this.viewMenuButton.click();
  106 |     }
  107 | 
  108 |     
  109 | 
