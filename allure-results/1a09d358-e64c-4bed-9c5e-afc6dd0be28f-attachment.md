...
> 72  |         await expect(this.storeCard).toBeVisible();
  ...
  81  |         //     }
  82  |         // }
  83  |         const storeCard = this.page.getByTestId('store-card').filter({ hasText: locationQuery });
  84  | 
  85  |         await storeCard.waitFor({ state: 'visible' });
  86  | 
  87  |         await storeCard.getByTestId('schedule-order').click();
  88  | 
  89  |     }
  90  |     async orderSummaryTitleIsVisible(): Promise<boolean> {
  91  |         return await this.orderSummaryTitle.isVisible();
  92  |     }
  93  | 
  94  |     async validateOrderSummary(): Promise<void> {
  95  |         this.viewMenuButton.click();
  96  |     }
  97  | 
  98  |     
  99  | 
